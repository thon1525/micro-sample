...
export class AuthController {
  ...
  public async SignUpWithEmail(
    ...
    try {
      ...
      const messageDetails = {
        ...
        verifyLink: `${
          getConfig(process.env.NODE_ENV).clientUrl
        }/verify-email?token=${verificationToken.emailVerificationToken}`,
    ...
    try {
      ...
      if (userDetail.role === 'USER') {
        response = await axios.post(
          `${getConfig(process.env.NODE_ENV).userServiceUrl}/v1/users`,
          data
        );
      } else {
        // ROLE: COMPANY
  ...
  public async GoogleAuth(@Query() role: string) {
    ...
    const url = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${
      getConfig(process.env.NODE_ENV).googleClientId
    }&redirect_uri=${
      getConfig(process.env.NODE_ENV).googleRedirectUri
    }&response_type=code&scope=profile email&state=${role}`;
    return { url };
  }
  ...
  public async GoogleAuthCallback(
    @Queries() queries: { code: string; state: string; [key: string]: any }
  ) {
